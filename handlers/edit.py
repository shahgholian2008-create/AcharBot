...
async def edit_photo_with_text(message: types.Message, state: FSMContext):
    """دکمه ویرایش با متن - از کاربر می‌خواهد عکس بفرستد"""
    await state.set_state(PhotoStates.waiting_for_edit_text)
    await message.answer(
        "📤 لطفاً عکس خود را ارسال کنید و **همراه آن** دستور متنی خود را بنویسید.\n\n"
        "🔹 مثال: عکس را بفرستید و در کپشن بنویسید: کارتونی\n\n"
...
async def handle_edit_photo(message: types.Message, state: FSMContext, bot):
    """پردازش عکس به همراه دستور متنی در کپشن"""
    # بررسی اینکه آیا کپشن (متن همراه عکس) وجود دارد
    if not message.caption:
        await message.answer(
            "❌ لطفاً عکس را **همراه با دستور متنی** در کپشن بفرستید.\n"
            "مثلاً: عکس را بفرستید و در کپشن بنویسید: کارتونی"
        )
        return

    prompt = message.caption.lower().strip()
    ...
    try:
        ...
        # ترمیم
        elif "ترمیم" in prompt or "restore" in prompt:
            ...
            if result_path and os.path.exists(result_path):
                ...
                await message.answer("❌ خطا در ترمیم عکس.")
                return
